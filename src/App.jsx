...
═══════════════════════════════════════════════════════════════ */
const INIT_EMPS = [
  {id:1,nom:"Martin Dubois",poste:"Ouvrier Espaces Verts",matricule:"OEV-001",photo:null,tel:"06 12 34 56 78",email:"m.dubois@societe.fr",dateEntree:"2022-03-15",
   infoPerso:{adresse:"12 rue des Lilas, 57100 Thionville",urgenceNom:"Claire Dubois",urgenceTel:"06 98 76 54 32",groupeSanguin:"A+",taille:"L",pointure:"43",notes:"Port de lunettes"},},
  {id:2,nom:"Sophie Bernard",poste:"Chef d'équipe",matricule:"CE-002",photo:null,tel:"06 23 45 67 89",email:"s.bernard@societe.fr",dateEntree:"2019-06-01",
   infoPerso:{adresse:"5 avenue Gambetta, 57000 Metz",urgenceNom:"Jean Bernard",urgenceTel:"06 87 65 43 21",groupeSanguin:"O+",taille:"M",pointure:"39",notes:"Allergie abeilles"},},
  {id:3,nom:"Karim Meziane",poste:"Conducteur d'engins",matricule:"CE-003",photo:null,tel:"06 34 56 78 90",email:"k.meziane@societe.fr",dateEntree:"2021-09-12",
   infoPerso:{adresse:"8 rue Victor Hugo, 57100 Thionville",urgenceNom:"Fatima Meziane",urgenceTel:"06 76 54 32 10",groupeSanguin:"B+",taille:"XL",pointure:"44",notes:""},},
  {id:4,nom:"Julie Pernet",poste:"Applicateur phyto",matricule:"AP-004",photo:null,tel:"06 45 67 89 01",email:"j.pernet@societe.fr",dateEntree:"2023-01-10",
   infoPerso:{adresse:"3 impasse des Roses, 57970 Yutz",urgenceNom:"Marc Pernet",urgenceTel:"06 65 43 21 09",groupeSanguin:"AB-",taille:"S",pointure:"37",notes:""},},
  {id:5,nom:"Thomas Roche",poste:"Responsable HSE",matricule:"HSE-005",photo:null,tel:"06 56 78 90 12",email:"t.roche@societe.fr",dateEntree:"2018-11-20",
   infoPerso:{adresse:"22 rue de la Paix, 57000 Metz",urgenceNom:"Marie Roche",urgenceTel:"06 54 32 10 98",groupeSanguin:"O-",taille:"L",pointure:"42",notes:"Sauveteur SST référent"},},
];
const POSTES_LIST=["Ouvrier Espaces Verts","Chef d'équipe","Conducteur d'engins","Applicateur phyto","Responsable HSE","Agent de maîtrise","Chauffeur PL"];
...
const INIT_FORMATIONS=[
  {id:1,titre:"Sauveteur Secouriste du Travail (SST)",categorie:"Sécurité",dureeValidite:24,employes:[1,2,3,4,5],obligatoire:true},
  {id:2,titre:"Habilitation électrique B0/H0",categorie:"Sécurité",dureeValidite:36,employes:[2,3,5],obligatoire:true},
  {id:3,titre:"CACES R482 – Engins de chantier",categorie:"Technique",dureeValidite:60,employes:[3],obligatoire:true},
  {id:4,titre:"Certiphyto Opérateur",categorie:"Phytosanitaire",dureeValidite:60,employes:[4,2],obligatoire:true},
  {id:5,titre:"Port des EPI – Tronçonneuse",categorie:"Sécurité",dureeValidite:12,employes:[1,2,3],obligatoire:true},
  {id:6,titre:"Travaux en hauteur / Nacelle",categorie:"Sécurité",dureeValidite:36,employes:[1,2,3],obligatoire:false},
  {id:7,titre:"Gestes et Postures (TMS)",categorie:"Santé",dureeValidite:24,employes:[1,2,3,4,5],obligatoire:false},
...
const INIT_MODES=[
  {id:1,ref:"MO-001",titre:"Utilisation débroussailleuse thermique",poste:["Ouvrier Espaces Verts","Chef d'équipe"],
   lignes:[
    {operation:"Préparation du matériel",moyens:"Débroussailleuse thermique, carburant, huile",risques:"Fuite carburant, incendie",prevention:"Vérifier les niveaux, pas de fuite"},
    {operation:"Mise en route",moyens:"Démarreur manuel, EPI complets",risques:"Projection, bruit excessif",prevention:"Distance > 15m des tiers, casque anti-bruit"},
    {operation:"Travaux de coupe",moyens:"Tête de coupe, fil nylon ou lame",risques:"Projection d'objets, coupure",prevention:"Visière obligatoire, zone balisée"},
    {operation:"Déplacement avec l'outil",moyens:"Protection de lame, moteur coupé",risques:"Coupure accidentelle",prevention:"Moteur coupé si déplacement > 10m"},
    {operation:"Nettoyage et rangement",moyens:"Chiffon, graisse, housse de protection",risques:"Brûlure (moteur chaud)",prevention:"Attendre refroidissement avant nettoyage"},
  ]},
  {id:2,ref:"MO-002",titre:"Application herbicide",poste:["Applicateur phyto"],
   lignes:[
    {operation:"Vérification météo",moyens:"Application météo, anémomètre",risques:"Dérive produit, contamination",prevention:"Vent < 3 Beaufort, pas de pluie prévue"},
    {operation:"Préparation du mélange",moyens:"Cuve, eau, produit homologué",risques:"Contact chimique, projection",prevention:"EPI complets : gants nitrile, lunettes, masque"},
    {operation:"Application",moyens:"Rampe, lance de traitement",risques:"Inhalation, contamination sol/eau",prevention:"Respecter doses homologuées, distance riverains 5m"},
    {operation:"Rinçage du matériel",moyens:"Eau claire x3, aire de remplissage étanche",risques:"Pollution environnementale",prevention:"Triple rinçage, eau de rinçage épandue sur zone traitée"},
    {operation:"Traçabilité",moyens:"Registre phytosanitaire papier ou numérique",risques:"Non-conformité réglementaire",prevention:"Remplir le registre immédiatement après intervention"},
  ]},
  {id:3,ref:"MO-003",titre:"Abattage arbre (diamètre < 40 cm)",poste:["Ouvrier Espaces Verts","Chef d'équipe"],
   lignes:[
    {operation:"Évaluation de l'arbre",moyens:"Observation visuelle, sonde si nécessaire",risques:"Chute imprévisible, arbre creux",prevention:"Évaluer inclinaison, état sanitaire, obstacles"},
    {operation:"Définition zone de chute",moyens:"Rubalise, barrières, guetteur",risques:"Chute sur tiers ou matériel",prevention:"Zone = 3x hauteur arbre, balisage obligatoire"},
    {operation:"Entaille de direction",moyens:"Tronçonneuse, cales d'abattage",risques:"Coupure, recul de lame",prevention:"Garde-chaîne en place, appui stable"},
    {operation:"Coupe d'abattage",moyens:"Tronçonneuse, crochets d'abattage",risques:"Chute de l'arbre vers opérateur",prevention:"Charnière conservée, voie de fuite dégagée"},
    {operation:"Débit et évacuation",moyens:"Tronçonneuse, broyeur, véhicule",risques:"Écrasement, effort lombaire",prevention:"Arbre immobile avant débit, technique de levage correcte"},
  ]},
  {id:4,ref:"MO-004",titre:"Balisage chantier voirie",poste:["Ouvrier Espaces Verts","Chef d'équipe","Conducteur d'engins"],
   lignes:[
    {operation:"Information préalable",moyens:"Déclaration mairie/gestionnaire voirie",risques:"Intervention non autorisée",prevention:"Toujours informer avant intervention"},
    {operation:"Mise en place signalisation",moyens:"Panneaux AK5, cônes K5a, barrières",risques:"Collision avec véhicule en transit",prevention:"Panneau à 150m minimum en amont"},
    {operation:"Protection de la zone",moyens:"Cônes, barrières Jersey si nécessaire",risques:"Intrusion de véhicules dans zone",prevention:"Zone complètement balisée avant début travaux"},
    {operation:"Travaux",moyens:"Gilets HV classe 3 obligatoires",risques:"Heurt par véhicule",prevention:"Guetteur si visibilité < 100m"},
    {operation:"Démontage balisage",moyens:"Véhicule de signalisation",prevention:"Démonter en partant de l'aval","risques":"Exposition trafic pendant démontage"},
  ]},
];
const INIT_HSE_DOCS=[
  {id:1,titre:"Livret d'accueil Sécurité",type:"Livret",categorie:"Sécurité",date:"2025-01-15",poste:"Tous",description:"Document remis à tout nouvel entrant",document:null,nomDoc:null,mimeType:null},
  {id:2,titre:"Document Unique (DUERP)",type:"Réglementaire",categorie:"Sécurité",date:"2025-03-01",poste:"Encadrement",description:"Evaluation des risques professionnels",document:null,nomDoc:null,mimeType:null},
  {id:3,titre:"Règlement Intérieur",type:"Réglementaire",categorie:"Général",date:"2024-09-01",poste:"Tous",description:"",document:null,nomDoc:null,mimeType:null},
  {id:4,titre:"FDS – Herbicides",type:"FDS",categorie:"Phytosanitaire",date:"2025-02-10",poste:"Applicateur phyto",description:"Fiche de données de sécurité produits",document:null,nomDoc:null,mimeType:null},
  {id:5,titre:"Procédure urgence et accidents",type:"Procédure",categorie:"Sécurité",date:"2025-01-15",poste:"Tous",description:"A afficher dans chaque véhicule",document:null,nomDoc:null,mimeType:null},
...
const INIT_DOC_CATS=["Sécurité","Santé","Phytosanitaire","Environnement","HSE","Général","Réglementaire","Formation","EPI","Urgence"];
const FORM_CATS=["Sécurité","Santé","Technique","Phytosanitaire","Environnement","Management"];
const MED_TYPES=["Visite périodique","Visite d'embauche","Visite de reprise","Surveillance renforcée","Visite spontanée"];
const APTITUDES=["Apte","Apte avec réserves","Inapte temporaire","Inapte définitif"];
const EPI_TYPES=["Casque","Lunettes / Visière","Protège-oreilles","Gants anti-coupure","Gants nitrile","Chaussures S3","Gilet haute visibilité","Harnais","Masque FFP2/FFP3","Combinaison","Jambières protection","Bottes"];
const HABL_TYPES=["Habilitation électrique B0","Habilitation électrique H0","CACES R482 Cat. A","CACES R482 Cat. B1","CACES R482 Cat. F","CACES R486 Cat. B","Travaux en hauteur","Certiphyto Opérateur","Certiphyto Décideur","AIPR Opérateur"];
...
function fStatus(fid,eid,duree,dates){
  ...
  if(!fd||!fd.date)return{s:"non",label:"Non réalisé",color:"#ef4444",bg:"#fee2e2"};
  const exp=getExpiry(fd.date,duree),diff=Math.round((exp-new Date())/86400000);
  if(diff<0)return{s:"exp",label:"Expiré",color:"#ef4444",bg:"#fee2e2",exp};
  if(diff<90)return{s:"soon",label:`${diff}j`,color:"#f59e0b",bg:"#fef3c7",exp};
  return{s:"ok",label:"À jour",color:"#16a34a",bg:"#dcfce7",exp};
}
function dStatus(nextDate){
  if(!nextDate)return{label:"Non planifiée",color:"#9ca3af",bg:"#f3f4f6"};
  const diff=Math.round((new Date(nextDate)-new Date())/86400000);
  if(diff<0)return{label:"Dépassée",color:"#ef4444",bg:"#fee2e2"};
...
function epiStatus(dateExp){
  ...
  if(diff<0)return{label:"Expiré",color:"#ef4444",bg:"#fee2e2"};
...
function idGen(){return Date.now()+Math.random().toString(36).slice(2,6);}
...
function Fld({label,value,onChange,type="text",placeholder="",style:sx={}}){
  return <div style={{marginBottom:12}}>
    {label&&<label className="label">{label}</label>}
    <input className="input" type={type} value={value||""} onChange={onChange} placeholder={placeholder} style={sx}/>
  </div>;
}
  ...
  return <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
    <div className="modal-box" style={{maxWidth:wide?900:680}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <button onClick={onClose} style={{background:"#f0f2ee",border:"none",borderRadius:10,width:34,height:34,cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
      </div>
      {children}
    </div>
  </div>;
...
function FileBtn({onFile,accept="*",children,small=false}){
  const ref=useRef();
  return <>
    <input ref={ref} type="file" accept={accept} style={{display:"none"}} onChange={async e=>{if(e.target.files[0]){const b=await toB64(e.target.files[0]);onFile(b,e.target.files[0].name,e.target.files[0].type);}e.target.value="";}}/>
    <button className={small?"btn-ghost":"btn-secondary"} style={{fontSize:12}} onClick={()=>ref.current.click()}>{children||"📎 Joindre fichier"}</button>
  </>;
}
...
function DocViewer({doc,nom,onClose}){
  ...
  return <Modal title={nom||"Document"} onClose={onClose} wide>
    ...
    {!isPdf&&!isImg&&<div style={{padding:40,textAlign:"center",color:"#6b7280"}}>
      ...
      <div style={{marginBottom:16}}>Aperçu non disponible pour ce type de fichier.</div>
      <button className="btn-primary" onClick={()=>downloadFile(doc,nom||"document")}>⬇️ Télécharger</button>
    </div>}
...
function DocChip({doc,nom,onView,onDelete,isAdmin}){
  if(!doc)return null;
  return <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"4px 10px",marginTop:6}}>
    <span style={{fontSize:12,color:"#15803d",fontWeight:600,cursor:"pointer"}} onClick={onView}>📎 {nom||"Document"}</span>
    {isAdmin&&<span style={{cursor:"pointer",color:"#dc2626",fontSize:13}} onClick={onDelete}>×</span>}
  </div>;
}
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#052e16,#14532d 60%,#166534)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{maxWidth:440,width:"100%"}}>
      <div style={{textAlign:"center",marginBottom:40}}>
        ...
        <div style={{color:"#86efac",fontSize:12,fontWeight:700,letterSpacing:4,marginBottom:8}}>RÉFÉRENTIEL MASE</div>
        <h1 style={{color:"#fff",fontSize:32,fontWeight:900,fontFamily:"'Fraunces',Georgia,serif",lineHeight:1.1}}>Espace Sécurité</h1>
        <p style={{color:"#bbf7d0",marginTop:8,fontSize:14}}>Santé · Sécurité · Environnement</p>
      </div>
      {!showAdmin?<div style={{display:"flex",flexDirection:"column",gap:14}}>
        <button onClick={onSalarie} style={{background:"rgba(255,255,255,0.12)",backdropFilter:"blur(8px)",border:"1.5px solid rgba(255,255,255,0.2)",borderRadius:16,padding:"20px 24px",cursor:"pointer",color:"#fff",display:"flex",alignItems:"center",gap:16,textAlign:"left"}}>
          ...
          <div><div style={{fontSize:16,fontWeight:700}}>Espace Salarié</div><div style={{fontSize:12,opacity:.7,marginTop:2}}>Consulter mes informations et documents</div></div>
        </button>
        <button onClick={()=>setShowAdmin(true)} style={{background:"rgba(255,255,255,0.07)",backdropFilter:"blur(8px)",border:"1.5px solid rgba(255,255,255,0.12)",borderRadius:16,padding:"20px 24px",cursor:"pointer",color:"#d1fae5",display:"flex",alignItems:"center",gap:16,textAlign:"left"}}>
          ...
          <div><div style={{fontSize:16,fontWeight:700}}>Espace Administrateur</div><div style={{fontSize:12,opacity:.7,marginTop:2}}>Gestion complète MASE</div></div>
        </button>
      </div>:<div style={{background:"rgba(255,255,255,0.1)",borderRadius:20,padding:28,backdropFilter:"blur(8px)"}}>
        <div style={{color:"#fff",fontWeight:800,fontSize:17,marginBottom:6,fontFamily:"'Fraunces',serif"}}>Connexion Administrateur</div>
        <div style={{color:"#86efac",fontSize:11,marginBottom:18}}>Indice : Admin + année en cours</div>
...
function Dashboard({employees,formations,formDates,medVisits,epiData,hablData,setPage,setSelEmp,isAdmin}){
  ...
  employees.forEach(emp=>{
    ...
    (medVisits[emp.id]||[]).forEach(v=>{const s=dStatus(v.prochaine);if(s.label==="Dépassée")alerts.push({type:"danger",nom:emp.nom,label:"Visite "+v.type,status:"Dépassée"});});
    (epiData[emp.id]||[]).forEach(e=>{const s=epiStatus(e.dateExpiration);if(s.label==="Expiré")alerts.push({type:"danger",nom:emp.nom,label:"EPI : "+e.nom,status:"Expiré"});});
    (hablData[emp.id]||[]).forEach(h=>{const s=epiStatus(h.dateExpiration);if(s.label==="Expiré")alerts.push({type:"danger",nom:emp.nom,label:"Habilitation : "+h.nom,status:"Expirée"});});
  });
  const dangers=alerts.filter(a=>a.type==="danger");
  const statCards=[{l:"Salariés",v:employees.length,i:"👷",c:"#eff6ff",t:"#1d4ed8"},{l:"Formations",v:formations.length,i:"🎓",c:"#f0fdf4",t:"#15803d"},{l:"Alertes",v:alerts.length,i:"⚠️",c:alerts.length>0?"#fff7ed":"#f9fafb",t:alerts.length>0?"#d97706":"#6b7280"},{l:"Critiques",v:dangers.length,i:"🚨",c:dangers.length>0?"#fef2f2":"#f9fafb",t:dangers.length>0?"#dc2626":"#6b7280"}];
  return <div>
    <div style={{marginBottom:24}}>
      <h1 style={{fontSize:26,fontWeight:900,color:"#1a2216",fontFamily:"'Fraunces',serif",marginBottom:4}}>Tableau de bord</h1>
      <p style={{color:"#6b7280",fontSize:14}}>Référentiel MASE – Espaces verts</p>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:24}}>
      {statCards.map(s=><div key={s.l} className="card" style={{background:s.c,boxShadow:"none",border:"1.5px solid transparent",padding:"18px 20px"}}>
    ...
    {alerts.length>0&&<div className="card" style={{border:`1.5px solid ${dangers.length>0?"#fca5a5":"#fde68a"}`,background:dangers.length>0?"#fff":"#fffbeb",marginBottom:20}}>
      <div style={{fontWeight:800,fontSize:15,color:dangers.length>0?"#dc2626":"#d97706",marginBottom:12}}>{dangers.length>0?"🚨":"⚠️"} {alerts.length} alerte(s) active(s)</div>
      <table className="table"><thead><tr><th>Salarié</th><th>Élément</th><th>Statut</th></tr></thead>
    ...
    <div className="card">
      <div style={{fontWeight:800,fontSize:15,color:"#1a2216",marginBottom:14}}>👷 Équipe ({employees.length} salariés)</div>
        ...
        {employees.map(emp=>{
          ...
          return <div key={emp.id} onClick={()=>{setSelEmp(emp.id);setPage("employe");}} style={{background:"#f8faf6",borderRadius:12,padding:"14px",cursor:"pointer",border:"1.5px solid #e5e7eb",display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3,alignItems:"flex-end"}}>
              ...
              {w>0&&<Badge color="#d97706" bg="#fef3c7">{w}⚠️</Badge>}
              {d===0&&w===0&&<Badge color="#16a34a" bg="#dcfce7">✓</Badge>}
            </div>
          </div>;
        })}
      </div>
  ...
  return <div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
      ...
      {isAdmin&&<div style={{display:"flex",gap:8}}>
        <button className="btn-secondary" onClick={()=>setEditingEmp(true)}>✏️ Modifier</button>
        <button className="btn-danger" onClick={()=>onDeleteEmp(emp.id)}>Supprimer</button>
      </div>}
    </div>

    <Tabs tabs={[["info","👤 Infos perso"],["formations","🎓 Formations"],["epi","🦺 EPI"],["habl","🏅 Habilitations"],["medical","🏥 Médical"],["docs","📄 Documents"]]} active={tab} onChange={setTab}/>
...
function InfoPerso({emp,isAdmin,onSave}){
  ...
  if(editing&&isAdmin)return <div className="card">
    ...
    <div className="split"><Fld label="Contact urgence – Tél." value={form.urgenceTel} onChange={set("urgenceTel")}/><FldSel label="Groupe sanguin" value={form.groupeSanguin} onChange={set("groupeSanguin")} options={["—","A+","A-","B+","B-","AB+","AB-","O+","O-"]}/></div>
    <div className="split"><Fld label="Taille vêtements" value={form.taille} onChange={set("taille")} placeholder="S/M/L/XL"/><Fld label="Pointure chaussures" value={form.pointure} onChange={set("pointure")} placeholder="42"/></div>
    <FldArea label="Notes médicales / restrictions" value={form.notes} onChange={set("notes")}/>
  ...
  return <div className="card">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      {isAdmin&&<button className="btn-secondary" style={{fontSize:12}} onClick={()=>{setForm({...ip});setEditing(true);}}>✏️ Modifier</button>}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20}}>
      {[["📧 Email",emp.email],["📞 Téléphone",emp.tel],["📅 Date d'entrée",fmtDate(emp.dateEntree)],["🏠 Adresse",ip.adresse],["🆘 Contact urgence",ip.urgenceNom?(ip.urgenceNom+(ip.urgenceTel?" – "+ip.urgenceTel:"")):"—"],["🩸 Groupe sanguin",ip.groupeSanguin||"—"],["👕 Taille vêtements",ip.taille||"—"],["👟 Pointure",ip.pointure||"—"],["📋 Notes",ip.notes||"—"]].map(([l,v])=><div key={l}>
...
function EmpFormations({emp,formations,formDates,onSave,isAdmin,onView}){
  ...
  if(formations.length===0)return <div className="card" style={{textAlign:"center",color:"#6b7280"}}>Aucune formation assignée.</div>;
  return <div>
    ...
    <table className="table" style={{background:"#fff",borderRadius:14,overflow:"hidden"}}>
      <thead><tr><th>Formation</th><th>Catégorie</th><th>Validité</th><th>Réalisée le</th><th>Échéance</th><th>Attestation</th>{isAdmin&&<th>Action</th>}</tr></thead>
      <tbody>
        {formations.map(f=>{
          const s=fStatus(f.id,emp.id,f.dureeValidite,formDates);
          ...
          return <tr key={f.id}>
            ...
            <td><Bdg s={s}/>{s.exp&&<div style={{fontSize:10,color:"#9ca3af",marginTop:3}}>le {s.exp.toLocaleDateString("fr-FR")}</div>}</td>
            <td>{fd.document?<div style={{display:"flex",gap:6}}>
              <button className="btn-ghost" style={{fontSize:11}} onClick={()=>onView({doc:fd.document,nom:fd.nomDoc})}>👁️ Voir</button>
              <button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(fd.document,fd.nomDoc)}>⬇️</button>
            </div>:<span style={{color:"#9ca3af",fontSize:12}}>—</span>}</td>
            {isAdmin&&<td><button className="btn-ghost" style={{fontSize:11}} onClick={()=>setEditF(f)}>✏️ Modifier</button></td>}
          </tr>;
        })}
      </tbody>
    </table>
...
function FormDateEditModal({formation,emp,formDates,onSave,onClose}){
  ...
  const [mime,setMime]=useState(cur.mimeType||null);
  return <Modal title={formation.titre} onClose={onClose}>
    <div style={{marginBottom:8,color:"#6b7280",fontSize:13}}>{formation.categorie} · Validité {formation.dureeValidite} mois · {emp.nom}</div>
    <Fld label="Date de réalisation" value={date} onChange={e=>setDate(e.target.value)} type="date"/>
    <div style={{marginBottom:12}}>
      <label className="label">Attestation / Certificat</label>
      <div style={{display:"flex",gap:10,alignItems:"center"}}>
        <FileBtn onFile={(b,n,m)=>{setDoc(b);setNom(n);setMime(m);}}>📎 Joindre attestation</FileBtn>
        {doc&&<div style={{display:"flex",gap:6,alignItems:"center"}}>
          <span style={{fontSize:12,color:"#15803d",fontWeight:600}}>📎 {nom}</span>
          <button className="btn-danger" style={{fontSize:11,padding:"3px 8px"}} onClick={()=>{setDoc(null);setNom(null);setMime(null);}}>×</button>
        </div>}
      </div>
    </div>
    <div style={{display:"flex",gap:10,marginTop:16}}>
      <button className="btn-secondary" onClick={onClose}>Annuler</button>
      <button className="btn-primary" onClick={()=>onSave({...formDates,[key]:{date,document:doc,nomDoc:nom,mimeType:mime}})}>Enregistrer</button>
    </div>
  </Modal>;
}
...
function EmpEpi({emp,epi,onSave,isAdmin}){
  ...
  const save=()=>{
    if(!form.nom)return;
    const updated=editIdx!==null?epi.map((e,i)=>i===editIdx?form:e):[...epi,form];
    onSave(updated);setShowForm(false);setEditIdx(null);setForm(empty);
  };
  ...
  return <div>
    ...
    {showForm&&<div className="card" style={{marginBottom:14,border:"1.5px solid #bbf7d0"}}>
      ...
      <div className="split"><FldSel label="Type d'EPI" value={form.nom} onChange={set("nom")} options={["",...EPI_TYPES]}/><Fld label="Référence / Marque" value={form.reference} onChange={set("reference")} placeholder="Ex: Uvex 9301"/></div>
      ...
      <Fld label="Date d'expiration / Remplacement prévu" value={form.dateExpiration} onChange={set("dateExpiration")} type="date"/>
    ...
    {epi.length===0?<div className="card" style={{textAlign:"center",color:"#6b7280"}}>Aucun EPI enregistré.</div>:
    <table className="table" style={{background:"#fff",borderRadius:14,overflow:"hidden"}}>
      <thead><tr><th>EPI</th><th>Référence</th><th>Taille</th><th>Attribution</th><th>Expiration / Remplacement</th>{isAdmin&&<th>Actions</th>}</tr></thead>
      <tbody>{epi.map((e,i)=>{const s=epiStatus(e.dateExpiration);return <tr key={i}>
        ...
        {isAdmin&&<td><div style={{display:"flex",gap:6}}>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>{setForm({...e});setEditIdx(i);setShowForm(true);}}>✏️</button>
          <button className="btn-danger" style={{fontSize:11}} onClick={()=>del(i)}>🗑</button>
        </div></td>}
      </tr>;})}
      </tbody>
    </table>}
...
function EmpHabl({emp,habl,onSave,isAdmin,onView}){
  ...
  const save=()=>{
    if(!form.nom)return;
    const updated=editIdx!==null?habl.map((h,i)=>i===editIdx?form:h):[...habl,form];
    onSave(updated);setShowForm(false);setEditIdx(null);setForm(empty);
  };
  ...
  return <div>
    ...
    {showForm&&<div className="card" style={{marginBottom:14,border:"1.5px solid #e9d5ff"}}>
      ...
      <div className="split"><FldSel label="Habilitation" value={form.nom} onChange={set("nom")} options={["",...HABL_TYPES]}/><Fld label="Organisme délivrant" value={form.organisme} onChange={set("organisme")} placeholder="Ex: APAVE"/></div>
      ...
      <div style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          ...
          {form.document&&<div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:12,color:"#7c3aed",fontWeight:600}}>📎 {form.nomDoc}</span><button className="btn-danger" style={{fontSize:11,padding:"3px 8px"}} onClick={()=>setForm(f=>({...f,document:null,nomDoc:null}))}>×</button></div>}
        </div>
      </div>
      <div style={{display:"flex",gap:10}}><button className="btn-secondary" onClick={()=>setShowForm(false)}>Annuler</button><button className="btn-primary" onClick={save}>Enregistrer</button></div>
    </div>}
    {habl.length===0?<div className="card" style={{textAlign:"center",color:"#6b7280"}}>Aucune habilitation enregistrée.</div>:
    <table className="table" style={{background:"#fff",borderRadius:14,overflow:"hidden"}}>
      ...
      <tbody>{habl.map((h,i)=>{const s=epiStatus(h.dateExpiration);return <tr key={i}>
        ...
        <td>{h.document?<div style={{display:"flex",gap:6}}>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>onView({doc:h.document,nom:h.nomDoc})}>👁️ Voir</button>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(h.document,h.nomDoc)}>⬇️</button>
        </div>:<span style={{color:"#9ca3af",fontSize:12}}>—</span>}</td>
        {isAdmin&&<td><div style={{display:"flex",gap:6}}>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>{setForm({...h});setEditIdx(i);setShowForm(true);}}>✏️</button>
          <button className="btn-danger" style={{fontSize:11}} onClick={()=>del(i)}>🗑</button>
        </div></td>}
      </tr>;})}
      </tbody>
...
function EmpMedical({emp,med,onSave,isAdmin,onView}){
  const [showForm,setShowForm]=useState(false);
  const [editIdx,setEditIdx]=useState(null);
  const empty={type:"Visite périodique",date:"",prochaine:"",medecin:"",aptitude:"Apte",notes:"",document:null,nomDoc:null};
  const [form,setForm]=useState(empty);
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  ...
  return <div>
    ...
    {showForm&&<div className="card" style={{marginBottom:14,border:"1.5px solid #bfdbfe"}}>
      <div style={{fontWeight:700,marginBottom:12}}>{editIdx!==null?"Modifier":"Nouvelle visite médicale"}</div>
      ...
      <Fld label="Médecin du travail" value={form.medecin} onChange={set("medecin")} placeholder="Dr. Nom"/>
      <FldArea label="Notes / Restrictions" value={form.notes} onChange={set("notes")} rows={2}/>
      <div style={{marginBottom:12}}>
        <label className="label">Document (avis d'aptitude, compte-rendu…)</label>
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          <FileBtn onFile={(b,n)=>setForm(f=>({...f,document:b,nomDoc:n}))}>📎 Joindre document</FileBtn>
          {form.document&&<div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:12,color:"#1d4ed8",fontWeight:600}}>📎 {form.nomDoc}</span><button className="btn-danger" style={{fontSize:11,padding:"3px 8px"}} onClick={()=>setForm(f=>({...f,document:null,nomDoc:null}))}>×</button></div>}
        </div>
      </div>
      <div style={{display:"flex",gap:10}}><button className="btn-secondary" onClick={()=>setShowForm(false)}>Annuler</button><button className="btn-primary" onClick={save}>Enregistrer</button></div>
    </div>}
    {med.length===0?<div className="card" style={{textAlign:"center",color:"#6b7280"}}>Aucune visite médicale enregistrée.</div>:
    <table className="table" style={{background:"#fff",borderRadius:14,overflow:"hidden"}}>
      <thead><tr><th>Type</th><th>Date</th><th>Médecin</th><th>Aptitude</th><th>Prochaine</th><th>Document</th>{isAdmin&&<th>Actions</th>}</tr></thead>
      <tbody>{med.map((v,i)=>{const s=dStatus(v.prochaine);const ac=v.aptitude==="Apte"?"#16a34a":v.aptitude==="Apte avec réserves"?"#d97706":"#dc2626";return <tr key={i}>
        ...
        <td>{v.document?<div style={{display:"flex",gap:6}}>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>onView({doc:v.document,nom:v.nomDoc})}>👁️ Voir</button>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(v.document,v.nomDoc)}>⬇️</button>
        </div>:<span style={{color:"#9ca3af",fontSize:12}}>—</span>}</td>
        {isAdmin&&<td><div style={{display:"flex",gap:6}}>
          <button className="btn-ghost" style={{fontSize:11}} onClick={()=>{setForm({...v});setEditIdx(i);setShowForm(true);}}>✏️</button>
          <button className="btn-danger" style={{fontSize:11}} onClick={()=>del(i)}>🗑</button>
        </div></td>}
      </tr>;})}
      </tbody>
...
function EmpDocs({emp,docs,onSave,isAdmin,onView,docCats}){
  const [showForm,setShowForm]=useState(false);
  const [form,setForm]=useState({titre:"",categorie:docCats[0]||"Autre",date:"",notes:"",document:null,nomDoc:null,mimeType:null});
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  const save=()=>{if(!form.titre.trim())return;onSave([...docs,form]);setShowForm(false);setForm({titre:"",categorie:docCats[0]||"Autre",date:"",notes:"",document:null,nomDoc:null,mimeType:null});};
  const del=i=>onSave(docs.filter((_,idx)=>idx!==i));
  const DOC_ICONS={"Formation":"🎓","Médical":"🏥","Entrée":"📋","Identité":"🪪","EPI":"🦺","Contrat":"📝","Autre":"📄"};
  return <div>
    {isAdmin&&<div style={{marginBottom:14}}><button className="btn-primary" onClick={()=>setShowForm(true)}>+ Ajouter un document</button></div>}
    {showForm&&<div className="card" style={{marginBottom:14,border:"1.5px solid #e9d5ff"}}>
      <div style={{fontWeight:700,marginBottom:12}}>Nouveau document salarié</div>
      <div className="split">
        <Fld label="Titre *" value={form.titre} onChange={set("titre")} placeholder="Ex: Attestation SST"/>
        <FldSel label="Catégorie" value={form.categorie} onChange={set("categorie")} options={docCats}/>
      </div>
      <div className="split">
      ...
      <div style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          <FileBtn onFile={(b,n,m)=>setForm(f=>({...f,document:b,nomDoc:n,mimeType:m}))}>📎 Joindre fichier</FileBtn>
          {form.document&&<div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:12,color:"#7c3aed",fontWeight:600}}>📎 {form.nomDoc}</span><button className="btn-danger" style={{fontSize:11,padding:"3px 8px"}} onClick={()=>setForm(f=>({...f,document:null,nomDoc:null,mimeType:null}))}>×</button></div>}
        </div>
      </div>
      <div style={{display:"flex",gap:10}}><button className="btn-secondary" onClick={()=>setShowForm(false)}>Annuler</button><button className="btn-primary" onClick={save}>Enregistrer</button></div>
    </div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
      {docs.map((d,i)=><div key={i} className="card" style={{padding:"16px"}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
          <div style={{fontSize:28,flexShrink:0}}>{DOC_ICONS[d.categorie]||"📄"}</div>
          <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
          {d.document&&<><button className="btn-ghost" style={{fontSize:11}} onClick={()=>onView({doc:d.document,nom:d.nomDoc})}>👁️ Voir</button><button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(d.document,d.nomDoc)}>⬇️</button></>}
...
function EmpEditModal({emp,onSave,onClose}){
  ...
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  return <Modal title={emp?.id?"Modifier le salarié":"Nouveau salarié"} onClose={onClose}>
    ...
    <div className="split"><Fld label="Nom complet *" value={form.nom} onChange={set("nom")} placeholder="Prénom NOM"/><FldSel label="Poste *" value={form.poste} onChange={set("poste")} options={POSTES_LIST}/></div>
    <div className="split"><Fld label="Matricule" value={form.matricule} onChange={set("matricule")} placeholder="OEV-001"/><Fld label="Téléphone" value={form.tel} onChange={set("tel")} placeholder="06 XX XX XX XX"/></div>
    <div className="split"><Fld label="Email" value={form.email} onChange={set("email")} type="email"/><Fld label="Date d'entrée" value={form.dateEntree} onChange={set("dateEntree")} type="date"/></div>
    <div style={{display:"flex",gap:10,marginTop:8}}>
      <button className="btn-secondary" onClick={onClose}>Annuler</button>
...
function FormationsPage({formations,setFormations,employees,formDates,setFormDates,isAdmin}){
  ...
  const emptyF={titre:"",categorie:"Sécurité",dureeValidite:24,employes:[],obligatoire:true};
  ...
  const toggleEmp=(formId,empId)=>{
    setFormations(formations.map(f=>{
      if(f.id!==formId)return f;
      const emps=f.employes.includes(empId)?f.employes.filter(e=>e!==empId):[...f.employes,empId];
      return{...f,employes:emps};
    }));
  };
  return <div>
    ...
    {showNew&&<div className="card" style={{marginBottom:16,border:"1.5px solid #bbf7d0"}}>
      ...
      <div className="split3"><Fld label="Titre *" value={newF.titre} onChange={setNF("titre")} placeholder="Nom de la formation"/><FldSel label="Catégorie" value={newF.categorie} onChange={setNF("categorie")} options={FORM_CATS}/><Fld label="Durée de validité (mois)" value={newF.dureeValidite} onChange={setNF("dureeValidite")} type="number" placeholder="24"/></div>
      <div style={{marginBottom:12}}><label className="label">Salariés concernés</label><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{employees.map(e=><label key={e.id} style={{display:"flex",alignItems:"center",gap:5,fontSize:13,cursor:"pointer",padding:"4px 10px",borderRadius:8,background:newF.employes.includes(e.id)?"#dcfce7":"#f3f4f6",border:newF.employes.includes(e.id)?"1.5px solid #86efac":"1.5px solid transparent"}}><input type="checkbox" checked={newF.employes.includes(e.id)} onChange={()=>setNewF(f=>({...f,employes:f.employes.includes(e.id)?f.employes.filter(id=>id!==e.id):[...f.employes,e.id]}))} style={{display:"none"}}/>{e.nom.split(" ")[0]}</label>)}</div></div>
      ...
      <div style={{display:"flex",gap:10}}><button className="btn-secondary" onClick={()=>setShowNew(false)}>Annuler</button><button className="btn-primary" onClick={saveNew}>Créer</button></div>
    </div>}
    ...
    {filtered.map(form=>{
      ...
      return <div key={form.id} className="card">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
          <div>
            ...
            <div style={{fontSize:12,color:"#6b7280",marginTop:3}}>
              ...
              <span style={{marginLeft:8}}>Validité {form.dureeValidite} mois</span>
              {form.obligatoire&&<span style={{marginLeft:8}}><Badge color="#7c3aed" bg="#f3e8ff">Obligatoire</Badge></span>}
            </div>
          </div>
          {isAdmin&&<div style={{display:"flex",gap:6}}>
            <button className="btn-ghost" style={{fontSize:11}} onClick={()=>setEditForm(form)}>✏️ Affecter</button>
            <button className="btn-danger" style={{fontSize:11}} onClick={()=>delForm(form.id)}>🗑</button>
          </div>}
        </div>
        ...
        {(expired>0||warn>0)&&<div style={{background:expired>0?"#fee2e2":"#fef3c7",borderRadius:8,padding:"6px 12px",fontSize:12,color:expired>0?"#dc2626":"#d97706",fontWeight:700}}>
          {expired>0?`🚨 ${expired} salarié(s) : action requise`:`⚠️ ${warn} salarié(s) : renouvellement proche`}
        </div>}
      </div>;
    })}
    {editForm&&<AffectFormModal formation={editForm} employees={employees} formDates={formDates} onToggle={toggleEmp} onSaveDate={dates=>{setFormDates(d=>({...d,...dates}));}} onClose={()=>setEditForm(null)}/>}
  </div>;
}

function AffectFormModal({formation,employees,formDates,onToggle,onSaveDate,onClose}){
  ...
  return <Modal title={"Affectation : "+formation.titre} onClose={onClose} wide>
    <div style={{color:"#6b7280",fontSize:13,marginBottom:16}}>{formation.categorie} · Validité {formation.dureeValidite} mois</div>
    <table className="table"><thead><tr><th>Salarié</th><th>Statut</th><th>Date réalisation</th><th>Attestation</th></tr></thead>
    <tbody>{relevant.map(emp=>{
      ...
      return <tr key={emp.id}>
        ...
        <td>{fd.document?<button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(fd.document,fd.nomDoc)}>⬇️ {fd.nomDoc}</button>:<span style={{color:"#9ca3af",fontSize:12}}>—</span>}</td>
      </tr>;
    })}</tbody></table>
...
/* ═══════════════════════════════════════════════════════════════
   MODES OPERATOIRES
═══════════════════════════════════════════════════════════════ */
function ModesPage({modes,setModes,employees,isAdmin}){
  ...
  if(selMode){
    ...
    return <ModeDetail mode={mode} modes={modes} setModes={setModes} employees={employees} isAdmin={isAdmin} onBack={()=>setSelMode(null)}/>;
  }
  return <div>
    <SectionHdr icon="🔧" title="Modes Opératoires" action={isAdmin&&<button className="btn-primary" onClick={()=>setShowNew(true)}>+ Nouveau mode opératoire</button>}/>
    {showNew&&<div className="card" style={{marginBottom:16,border:"1.5px solid #fde68a"}}>
      <div style={{fontWeight:700,marginBottom:14}}>Nouveau mode opératoire</div>
      <div className="split3"><Fld label="Référence" value={newM.ref} onChange={setNM("ref")} placeholder="MO-005"/><Fld label="Titre *" value={newM.titre} onChange={setNM("titre")} placeholder="Nom de l'opération"/><div></div></div>
      <div style={{marginBottom:12}}><label className="label">Postes concernés</label><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{POSTES_LIST.map(p=><label key={p} style={{display:"flex",alignItems:"center",gap:5,fontSize:12,cursor:"pointer",padding:"4px 10px",borderRadius:8,background:newM.poste.includes(p)?"#fef3c7":"#f3f4f6",border:newM.poste.includes(p)?"1.5px solid #fde68a":"1.5px solid transparent"}}><input type="checkbox" checked={newM.poste.includes(p)} onChange={()=>setNewM(m=>({...m,poste:m.poste.includes(p)?m.poste.filter(x=>x!==p):[...m.poste,p]}))} style={{display:"none"}}/>{p}</label>)}</div></div>
      <div style={{marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><label className="label" style={{margin:0}}>Tableau des opérations</label><button className="btn-ghost" style={{fontSize:11}} onClick={addLigne}>+ Ajouter ligne</button></div>
        <div style={{overflowX:"auto"}}>
          <table className="table" style={{minWidth:700}}>
            <thead><tr><th style={{width:"22%"}}>Opération</th><th style={{width:"25%"}}>Moyens mis en oeuvre</th><th style={{width:"25%"}}>Risques</th><th style={{width:"25%"}}>Moyens de prévention</th><th style={{width:"3%"}}></th></tr></thead>
            <tbody>{newM.lignes.map((l,i)=><tr key={i}>
              ...
              <td><button className="btn-danger" style={{fontSize:12,padding:"4px 8px"}} onClick={()=>delLigne(i)}>×</button></td>
            </tr>)}</tbody>
          </table>
        </div>
      </div>
      <div style={{display:"flex",gap:10,marginTop:12}}><button className="btn-secondary" onClick={()=>setShowNew(false)}>Annuler</button><button className="btn-primary" onClick={saveNew}>Créer</button></div>
    </div>}
      ...
      {modes.map(m=><div key={m.id} className="card" style={{cursor:"pointer"}} onClick={()=>setSelMode(m.id)}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            <Badge color="#6b7280" bg="#f3f4f6">{m.lignes.length} étapes</Badge>
            {isAdmin&&<button className="btn-danger" style={{fontSize:11,padding:"4px 8px"}} onClick={e=>{e.stopPropagation();del(m.id);}}>🗑</button>}
          </div>
        </div>
      </div>)}
    </div>
...
function ModeDetail({mode,modes,setModes,employees,isAdmin,onBack}){
  const [editing,setEditing]=useState(false);
  ...
  return <div>
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
      ...
      {isAdmin&&!editing&&<button className="btn-secondary" onClick={()=>{setForm({...mode,lignes:mode.lignes.map(l=>({...l}))});setEditing(true);}}>✏️ Modifier</button>}
    ...
    {editing&&<div className="card" style={{marginBottom:14}}>
      <div className="split3">
        <Fld label="Référence" value={form.ref} onChange={setF("ref")}/>
        <Fld label="Titre" value={form.titre} onChange={setF("titre")}/>
        <div><label className="label">Postes</label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{POSTES_LIST.map(p=><label key={p} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,cursor:"pointer",padding:"3px 8px",borderRadius:6,background:form.poste.includes(p)?"#fef3c7":"#f3f4f6"}}><input type="checkbox" checked={form.poste.includes(p)} onChange={()=>setForm(m=>({...m,poste:m.poste.includes(p)?m.poste.filter(x=>x!==p):[...m.poste,p]}))} style={{display:"none"}}/>{p.split(" ")[0]}</label>)}</div></div>
      </div>
    </div>}
      ...
      <div style={{overflowX:"auto"}}>
        <table className="table" style={{minWidth:800}}>
          <thead><tr>
            ...
            <th style={{width:"22%",background:"#14532d",color:"#fff"}}>Opération</th>
            ...
            <th style={{width:"24%",background:"#15803d",color:"#fff"}}>Moyens de prévention</th>
          ...
          <tbody>
            {cur.lignes.map((l,i)=>editing?<tr key={i}>
              ...
              <td><div style={{display:"flex",flexDirection:"column",gap:3}}>{(l.risques||"").split(",").map((r,j)=>r.trim()&&<div key={j} style={{display:"flex",gap:6,alignItems:"flex-start"}}><span style={{color:"#dc2626",fontSize:14,lineHeight:1.2}}>●</span><span style={{fontSize:12}}>{r.trim()}</span></div>)}</div></td>
              <td><div style={{display:"flex",flexDirection:"column",gap:3}}>{(l.prevention||"").split(",").map((r,j)=>r.trim()&&<div key={j} style={{display:"flex",gap:6,alignItems:"flex-start"}}><span style={{color:"#16a34a",fontSize:14,lineHeight:1.2}}>✓</span><span style={{fontSize:12}}>{r.trim()}</span></div>)}</div></td>
            </tr>)}
          </tbody>
        </table>
      </div>
    ...
    <div style={{marginTop:16,padding:14,background:"#fafff8",borderRadius:12,border:"1px solid #dcfce7",fontSize:12,color:"#6b7280"}}>
      Séparez les risques et prévention par des virgules pour les afficher sous forme de liste.
    </div>
  </div>;
}
...
function DocumentsPage({docs,setDocs,isAdmin,docCats,setDocCats}){
  ...
  const [viewDoc,setViewDoc]=useState(null);
  const empty={titre:"",type:"",categorie:docCats[0]||"Sécurité",date:"",poste:"Tous",description:"",document:null,nomDoc:null,mimeType:null};
  const [form,setForm]=useState(empty);
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  ...
  const CICONS={"Sécurité":"🛡️","Santé":"💊","Phytosanitaire":"🌿","Environnement":"🌍","HSE":"⚙️","Général":"📄","Réglementaire":"⚖️","Formation":"🎓","EPI":"🦺","Urgence":"🚨"};
  return <div>
    {viewDoc&&<DocViewer doc={viewDoc.doc} nom={viewDoc.nom} onClose={()=>setViewDoc(null)}/>}
    <SectionHdr icon="📁" title="Documents HSE" action={isAdmin&&<div style={{display:"flex",gap:8}}>
      <button className="btn-secondary" style={{fontSize:12}} onClick={()=>setShowCatEditor(true)}>⚙️ Catégories</button>
    ...
    {showNew&&<div className="card" style={{marginBottom:16,border:"1.5px solid #e9d5ff"}}>
      ...
      <div className="split"><Fld label="Titre *" value={form.titre} onChange={set("titre")} placeholder="Nom du document"/><FldSel label="Catégorie" value={form.categorie} onChange={set("categorie")} options={docCats}/></div>
      <div className="split3"><Fld label="Type" value={form.type} onChange={set("type")} placeholder="Ex: Procédure, FDS…"/><Fld label="Date" value={form.date} onChange={set("date")} type="date"/><FldSel label="Destiné à" value={form.poste} onChange={set("poste")} options={["Tous",...POSTES_LIST]}/></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
      {filtered.map(d=><div key={d.id} className="card" style={{padding:16}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:10}}>
          <div style={{width:46,height:46,borderRadius:12,background:"#f0fdf4",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{CICONS[d.categorie]||"📄"}</div>
          <div style={{flex:1,minWidth:0}}>
            ...
            {d.description&&<div style={{fontSize:11,color:"#9ca3af",marginTop:3,lineHeight:1.4}}>{d.description}</div>}
          </div>
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          <Badge color="#16a34a" bg="#dcfce7">{d.categorie}</Badge>
          {d.document&&<><button className="btn-ghost" style={{fontSize:11}} onClick={()=>setViewDoc({doc:d.document,nom:d.nomDoc})}>👁️ Voir</button><button className="btn-ghost" style={{fontSize:11}} onClick={()=>downloadFile(d.document,d.nomDoc)}>⬇️</button></>}
          {isAdmin&&<button className="btn-danger" style={{fontSize:11}} onClick={()=>del(d.id)}>🗑</button>}
        </div>
      </div>)}
    </div>
  </div>;
...
function CatEditor({cats,onSave,onClose}){
  ...
  return <Modal title="Gérer les catégories" onClose={onClose}>
    <div style={{marginBottom:16}}>
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <input className="input" value={newCat} onChange={e=>setNewCat(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Nouvelle catégorie…" style={{flex:1}}/>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
        {list.map((c,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,background:"#f0f2ee",borderRadius:8,padding:"5px 12px"}}>
          ...
          <button onClick={()=>del(i)} style={{background:"none",border:"none",cursor:"pointer",color:"#dc2626",fontSize:16,lineHeight:1}}>×</button>
        </div>)}
      </div>
    </div>
...
function EmpsPage({employees,setEmployees,formations,formDates,medVisits,epiData,hablData,isAdmin,onSelectEmp}){
  ...
  const alerts_count=emp=>{
    let d=0,w=0;
    formations.filter(f=>f.employes.includes(emp.id)).forEach(f=>{const s=fStatus(f.id,emp.id,f.dureeValidite,formDates);if(s.s==="exp"||s.s==="non")d++;else if(s.s==="soon")w++;});
    (medVisits[emp.id]||[]).forEach(v=>{if(dStatus(v.prochaine).label==="Dépassée")d++;});
    (epiData[emp.id]||[]).forEach(e=>{if(epiStatus(e.dateExpiration).label==="Expiré")d++;});
    return{d,w};
  };
  return <div>
    {showNew&&<EmpEditModal emp={{}} onSave={addEmp} onClose={()=>setShowNew(false)}/>}
    <SectionHdr icon="👷" title="Salariés" action={isAdmin&&<button className="btn-primary" onClick={()=>setShowNew(true)}>+ Nouveau salarié</button>}/>
    <input className="input" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher un salarié…" style={{maxWidth:360,marginBottom:16}}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
      {filtered.map(emp=>{const{d,w}=alerts_count(emp);return <div key={emp.id} className="card" style={{cursor:"pointer",padding:16}} onClick={()=>onSelectEmp(emp.id)}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
            {d>0&&<Badge color="#dc2626" bg="#fee2e2">🚨 {d}</Badge>}
            {w>0&&<Badge color="#d97706" bg="#fef3c7">⚠️ {w}</Badge>}
...
export default function App(){
  ...
  const [role,setRole]=useState(null); // null | "admin" | "salarie"
  ...
  const save=useCallback(async(k,v)=>{await sSave(k,v);},[]);
  const mk=(key,setter)=>async v=>{setter(v);await save(key,v);};
  ...
  const saveMed=async(eid,v)=>{const u={...medVisits,[eid]:v};setMedVisitsR(u);await save("ev2-med",u);};
  const saveEmpDocs=async(eid,v)=>{const u={...empDocs,[eid]:v};setEmpDocsR(u);await save("ev2-empdocs",u);};
  const saveEpi=async(eid,v)=>{const u={...epiData,[eid]:v};setEpiDataR(u);await save("ev2-epi",u);};
  const saveHabl=async(eid,v)=>{const u={...hablData,[eid]:v};setHablDataR(u);await save("ev2-habl",u);};
  ...
  const NAV=[
    ...
    {k:"employes",i:"👷",l:"Salariés"},
    {k:"formations",i:"🎓",l:"Formations"},
    {k:"modes",i:"🔧",l:"Modes opératoires"},
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:"#f0f2ee"}}>
    ...
    <div className="main-content" style={{marginLeft:220,flex:1,padding:"28px 32px",maxWidth:1200,boxSizing:"border-box"}}>
      ...
      page==="modes"?<ModesPage modes={modes} setModes={setModes} employees={employees} isAdmin={isAdmin}/>:
